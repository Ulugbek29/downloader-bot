...
async def link_handler(Mbot, message):
    ...
    try:
        ...
        if get_api['status'] and "Неверный URL-адрес видео TikTok." in get_api['mess']: 
           return await message.reply("К сожалению, неверный URL-адрес видео TikTok. Пожалуйста, попробуйте еще раз :) ")
           ...
           try:
              if "MP3" in get_api['links'][0]['t']:
                 try:
                     ...
                     pass 
              dump_file = await message.reply_video(get_api['links'][0]['a'], caption="Спасибо за использование - @OKAsave_bot")
           except KeyError:
